 ...
 * turned into a trend via `computeTrend`.
 *
 * The trend is computed relative to the newest point in the series rather than
 * the wall clock, so the range windows (30d / 90d / …) always have data no
 * matter when the demo is viewed.
 */
export function getDemoTrend(
  ...
  const trend = computeTrend(series, range, now);

  return { series, trend, range, now };
}
