...
/**
 * Typed data client for the analytics dashboard.
 *
 * When `NEXT_PUBLIC_API_URL` is set, it fetches live analyses and stored
 * history from the Beacon API. Otherwise — and whenever a request fails — it
 * falls back to the local `@beacon/core` + `@beacon/analytics` demo fixtures so
 * the UI is always populated. Safe to call from Server Components.
 */
...
interface ApiTrend {
  /** Pre-built health series, or raw `{ score, collectedAt }` history items. */
  series?: HealthPoint[];
...
/**
 * Fetch the health history + trend for `owner/repo`. When live, it reads the
 * stored history from `…/trend?range=`; on any failure it falls back to the
 * synthesized demo history. Either way the full series is returned so the chart
 * can re-window client-side.
 */
